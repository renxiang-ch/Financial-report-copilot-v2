...
- **2026-09-10/11**：**评测统一 + hook 归位**（devlog 006）。新增 `copilot/v2/eval/score.py` —— 一个数据集、一个 scorer、三个实现（v1_loop/graph 实跑，generic_agent replay），指标分 steps-independent（三方可比）/ steps-dependent（仅有 tool trace 的）两块。v1_loop 精确复现 Phase 0b 作为 scorer 验收。**统一 scorer 立刻抓到 2 个 parity 测不出的 Phase 1 回归**：EPS `:,.0f` 显示成 6、`retrieve_text` `[:700]` 丢 76% 检索文本（去截断后 token 仅 +5.2%，仍比 v1_loop 低 18%）。另修 `runner` 的 "step output 永远是 dict" 不变式（harness 8+ 处无保护依赖它）。middleware hook 归位：`_trim`/`_resolve`/`_guard` → `@before_agent`（框架原生"每轮一次"，手写守卫 5→2 处），`_resolve` 自带 `state_schema`，`build.py` 去 `GraphState`。**三方在 eval_set 可答题上完全打平**（Tier1/Tier2/retrieval judge 逐项 100%/100%/2.57）—— 该集已饱和。`pytest` 169/0
- **2026-09-11**：**Phase 3 开工**（devlog 007）。**3.1 PostgresSaver** —— `COPILOT_CHECKPOINTER=postgres` 时持 `ConnectionPool`（`from_conn_string` 是上下文管理器，不适合模块级单例）；**跨进程续答验证通过**：全新进程只问「How dependent is it on Apple?」，继承 CRUS + FY2024 答 87.0%。**3.3 `ModelFallbackMiddleware`**（`COPILOT_FALLBACK_MODELS`，只在异常上切）smoke 通过。**3.4 grounding 回边** —— `@after_model(can_jump_to=["model"])` 把 v1 的「事后贴标签」变成「退回重做」，4 个单测通过；但**开关 A/B 逐项相同**，现有集 `grounding_flagged` 本就为 0，**评测上是 no-op**。回归：`pytest` 173/0 · `score --impl graph` Tier1 17/17 · `ab_compare` eval_set 30/30+30/30。**3.2 卡 `LANGSMITH_API_KEY`**（→ 已于 09-13 完成，见下一条）
- **2026-09-12**：**LangChain 重构收尾 + 3.2 补做**。新增 [devlog 008](008-langchain-refactor-retrospective.md) —— 按目标 ① 的六个维度结算（状态管理 / 工具调用 / 错误恢复 / 持久化 / 可观测 / 可扩展）+ 最终数字 + 13 条踩坑（每条标"为什么闸门没抓到"）+ **Phase 4 的四条入场条件**。**3.2 LangSmith 打通**：文档说的"纯环境变量零代码"在本项目不成立 —— `.env` 的值 SDK 看不见（pydantic-settings 不导出，和 004 D1 同根因）、`get_env_var` 是 `@lru_cache` 的（光桥接不够，要清缓存），**两条都静默失败**。新增 `copilot/v2/observability.py`（`enable_tracing()` + `tracing_project()`）+ `scripts/smoke_langsmith.py`。**验收 PASS：6 个 hook 各自出 span**，且 span 树把"`before_agent` 每轮 1 次 / `wrap_model_call` 每次调用"从推断变成实测。**trace 立刻抓到一个真 bug**：`ToolRetryMiddleware` 顺序装反（retry 在 outer）→ 内层 error middleware 先转 ToolMessage → **retry 从未触发，死代码跑了整个 Phase 1/2/3**；而且交换顺序是错的修法（三个 kind 全是确定性参数错误，机械重试同参数只多跑 3 倍）。**已移除 retry middleware**，`retryable` → `model_correctable`，补 `tests/test_tool_error_recovery.py`（5 个行为测试，验过"有牙"）。`pytest` **186/0**
- **下一步动作**：**3.5 的设计决策** —— 澄清改真 `interrupt()` 后 `run()` 对歧义问题不再返回答案，`score`/`ab_compare` 会判为失败，而 UI 在 Phase 5.F（已 park）。要么做兼容层（中断时把澄清文本当 answer + 标志位），要么整体推 Phase 5。**评测优先级已连续三次被证明应上移**：扩 `undisclosed` 真陷阱题（1→8~10 道）+ 建 Tier 4 —— 否则 Phase 3/4 都是「做了但证明不了」
- **阻塞项**：无
...
| 006 | 评测统一 —— 一个数据集 / 一个 scorer / 三个实现 + middleware hook 归位 | done | [006](006-unified-eval.md) | 三方同尺子首次可比；抓到 2 个 parity 测不出的回归；三方 eval_set 可答题打平（该集已饱和）；pytest 169/0 |
| 007 | Phase 3 — 框架能力成熟化（持久化 / 可观测 / 降级 / grounding 回边 / 按需澄清） | active | [007](007-phase3-maturity.md) | 3.1 跨进程续答 ✅ · 3.3 模型降级 ✅ · 3.4 回边代码+单测 ✅（评测上 no-op）· **3.2 LangSmith ✅ 6 hook 各自出 span，并抓到 retry middleware 装反的死代码** · 3.5 推 Phase 5.F；pytest 186/0 |
| 008 | LangChain 重构复盘 —— 六维度结算 + Phase 4 入场条件 | done | [008](008-langchain-refactor-retrospective.md) | 目标 ① 的结算单：准确率持平（天花板早到）、token −18%、拿到 4 件 v1 做不到的事、13 条踩坑；**量化了仪器噪声底（judge ±0.14 / citation ±4）> Phase 3 可测效应**，据此定出 Phase 4 的 4 条入场条件 |
...
| `src/copilot/v2/observability.py` | v2 | LangSmith 接入：`enable_tracing()` 把 `.env` 的 `LANGSMITH_*` 桥接进 `os.environ` 并清 SDK 的 `lru_cache`（两条都是**静默**失败点）；`tracing_project()` 给 eval 分项目 + 每题盖 item id |
| `scripts/smoke_langsmith.py` | v2 | 3.2 验收：跑一道真题 → 从 API 把 trace 读回 → 打印 span 层级树 + 逐个核对 6 个 hook。需真 key，故不在 `pytest` 里 |
| `tests/test_tool_error_recovery.py` | v2 | `ToolError` 端到端行为闸门（工具体只执行 1 次 / 披露带 hint / 非 ToolError 传播 / 栈里没有 retry middleware）。用 fake model，零成本。**这个文件的缺失让 retry 装反藏了三个 Phase** |
| `src/copilot/v2/eval/score.py` | v2 | **统一 scorer**：一个数据集 / 一个 scorer / 三个实现（`--impl {v1_loop,graph,generic_agent}`）。复用 `harness.score_item`，指标分 steps-independent / steps-dependent 两块 |
| `src/copilot/v2/eval/ab_compare.py` | v2 | v1_loop vs graph 端到端 **parity**（一致性，非绝对分）。编排改动要和 `score.py` 一起跑 |
