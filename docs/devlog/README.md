...
- **2026-09-07**：devlog 004 **Step 3 完成** —— 路由 middleware（`graph/middleware.py`：`@before_model` refuse 短路 + `@wrap_model_call` force_tool）。`ab_compare` router 集 12 题对 v1_loop：**refusal 12/12**、`dependency` 首轮 pin `graph_query` 确认；citations 7/12（5 处差异经逐条复查为最小 wrapper 的 `retrieve_text` 年份范围 + gpt-4o-mini 工具参数非确定性，非路由回归，归 Step 4 / Phase 1）。回归：eval_set.json 仍 30/30 citations+refusal，`pytest` 143/0
- **下一步动作**：devlog 004 Step 4（carried-slots / active-context middleware，需 `state_schema=`）→ 跑 multiturn eval 集，顺带收 Step 3 那 4 处年份范围差异；然后 Step 5-6（clarify / history-trim）；再让 graph 过 tier3 + defects + multiturn
- **阻塞项**：无
...
| 004 | Phase 2 — Agent loop 端口（→ `create_agent`） | active | [004](004-agentloop-port.md) | Step 1+3 完成：eval_set.json 30/30 匹配；router 集 refusal 12/12 + force_tool 确认 |
...
| `src/copilot/v2/orchestration/graph/` | v2 | LangChain 重制版：`tools.py`（@tool 包装）/ `build.py`（create_agent）/ `middleware.py`（路由：refuse 短路 + force_tool）/ `runner.py`（驱动+重建 v1 形状）|
