...
> 规格原本散在 §2 / §3.1 / §4.1 / 附录，这里汇总成可直接引用的形态。

### 0.1 实际配置

| 维度 | 事实 |
|---|---|
| Agent | Codex CLI，`codex exec`（每题一个全新进程）|
| 模型 | **`gpt-5.6-sol`**，`-m` 显式钉死（不用默认，保证可复现）|
| 语料 | **58 份真实 10-K**，172MB，**6 家公司**（AAPL / AVGO / CRUS / GLW / QRVO / SWKS），SEC EDGAR 原始 **inline-XBRL HTML**，**未做任何预处理** |
| 数据访问 | `-C baseline/raw_filings`，一个目录 |
| 工具 | **通用 shell**（Read / Grep / Bash / ripgrep），**零领域基础设施** —— 无 SQL、无向量库、无预切块、无供应链图 |
| 检索方式 | **agent 自主检索** —— 自己写 `rg -o -P` 在大文件里定位，从不整份读入 |
| 联网 | **关**（不传 `--search`）|
| 沙箱 | `-s read-only` |
| 隔离 | 题间无记忆；工作根目录钉在语料目录，看不到 v2 的代码与 prompt |
| 系统提示 | 三句话，无领域指导：*"You are a financial analyst. This directory contains SEC 10-K filings... Question: {q}... If you consult a filing, name which file you used."* |
| 评分 | 与 v1_loop / graph **同一 scorer**（`copilot.v2.eval.score`），同容差带、同拒答检测器、同 judge rubric |

### 0.2 一段话描述（论文用）

> 为量化领域工程的边际价值，我们设置了一条**零定制基线**。它使用通用编码 agent（Codex CLI，模型钉死 `gpt-5.6-sol`）在一个包含 6 家公司、58 份真实 SEC 10-K 原始文件（172MB inline-XBRL HTML，未经任何预处理）的目录上作答。该 agent 只有通用 shell 工具（ripgrep / bash / 文件读取），**自行完成检索**——语料远超上下文窗口，它通过正则定位而非全文载入。系统提示仅三句话，不含任何领域指导；联网搜索关闭；每道题启动独立进程，题间无状态。评分复用与 v1_loop 完全相同的容差带、拒答检测器与 LLM judge 口径。

### 0.3 它**不是**什么（预先排除三种误读）

- ❌ **不是 long-context 基线** —— 没有把全文塞进上下文，172MB 也不可能
- ❌ **不是 RAG 基线** —— 无 embedding、无向量检索，是 agent 自己 grep
- ❌ **不是"ChatGPT 随便问问"** —— 无联网、语料受限、模型钉死、进程隔离，可复现

它测的问题是：**"一个通用 agent 用自己的工具做检索，能否替代为这个领域专门搭的摄取 + 检索 + 工具管线？"** 不是"大上下文能否替代检索"。

### 0.4 控制了什么 / 混淆了什么（必须自己讲出来）

**控制住的**：同一批题、同一评分口径、同样的原始事实来源（都是 SEC 10-K）、无联网、题间无记忆。

**没控制住的**：
- **模型档次混淆** —— generic_agent 用 gpt-5.6-sol（2026 代推理模型），v1_loop / graph 用 gpt-4o-mini（2024 代小模型）。**分差里混了「架构」和「模型」两个因素。** §4.5 提过解法（加 `v1_loop@strong`，同模型不同脚手架），因 v1_loop 已 100% 无 headroom 未执行 —— 但这恰恰说明**当前对比无法隔离架构变量**。
- **语料范围不同** —— 基线只有 6 家公司的 10-K；v1 库有 15 家 + 供应链边 + 文本切块。
- **n=1** —— §4.4 要求跑 3 趟报方差，未执行。
